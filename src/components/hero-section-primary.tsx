...
const HeroSectionPrimary = ({
 ...
 <span data-slot="italic">Thrive in Kenya.</span>
  ...
  return (
    <div
      className={clsx(
        'hero-section-primary relative flex w-full',
        compact ? 'min-h-70 sm:min-h-78 lg:min-h-[calc(70dvh-96px)]' : 'min-h-100 sm:min-h-112 lg:min-h-[calc(100dvh-96px)]',
        className
      )}
    >
      ...
      <div className="relative container flex flex-1 py-22 2xl:py-26">
        <div className="mx-auto flex w-full max-w-5xl flex-col items-center gap-22 justify-between">
          <div className="flex max-w-4xl flex-col items-center text-center 2xl:max-w-5xl">
            <FadeIn transition={{ delay: 0.1, duration: 1 }}>
              <Heading
                level={1}
          ...
          <FadeIn className="flex w-full" transition={{ delay: 0.4, duration: 1 }}>
            <HeroSearchPrimaryHome listingCategories={listingCategories} />
          </FadeIn>
        </div>
      </div>
    </div>
